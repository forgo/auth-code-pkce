...
describe('storage/session', () => {
  ...
  it('should store and retrieve values with namespace prefix', () => {
    ...
    expect(sessionStorage.getItem('@auth-code-pkce::key')).toBe('value');
  });
  ...
  it('should remove values', () => {
    ...
    expect(sessionStorage.getItem('@auth-code-pkce::key')).toBeNull();
  });
  ...
  it('should use custom namespace', () => {
    const storage = createSessionStorageAdapter('custom-ns');
    storage.set('key', 'value');

    expect(sessionStorage.getItem('custom-ns::key')).toBe('value');
    expect(sessionStorage.getItem('@auth-code-pkce::key')).toBeNull();
  });

  it('should isolate different namespaces', () => {
    const storage1 = createSessionStorageAdapter('ns1');
    const storage2 = createSessionStorageAdapter('ns2');
